    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration < opt.densify_until_iter:
            ...
            loss = (
                ...
                + normal_loss
                + dist_loss
                + torch.abs(triangles.get_sigma).mean()
                + 1 / (torch.abs(triangles.get_opacity).mean() * 0.05)
            )  # we push the triangles towards being more opaque and solid
        ...
        with torch.no_grad():
            ...
            if (
                ...
                if number_of_views < 250:
                    dead_mask = torch.logical_or(
                        (
                            triangles.importance_score < args.importance_threshold
                        ).squeeze(),
                    ...
                    if not new_round:
                        ...
                        dead_mask = (
                            triangles.get_opacity <= args.opacity_dead
                        ).squeeze()
                ...
                triangles.add_new_gs(
                    cap_max=opt.max_shapes, oddGroup=oddGroup, dead_mask=dead_mask
                )
            ...
            if (
                ...
                if iteration > 28000:
                    mask_test = (
                        triangles.get_sigma >= 1.0
                    )  # we remove the triangles that are not solid enough
                    dead_mask = torch.logical_or(dead_mask, mask_test.squeeze())
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    start_event.record()
                    image = torch.clamp(
                        renderFunc(viewpoint, scene.triangles, *renderArgs)["render"],
                        0.0,
                        1.0,
                    )
                    end_event.record()
        ...
        if tb_writer:
            tb_writer.add_histogram(
                "scene/opacity_histogram", scene.triangles.get_opacity, iteration
            )
            tb_writer.add_scalar(
                "total_points", scene.triangles.get_triangles_points.shape[0], iteration
            )
        torch.cuda.empty_cache()
