...
def densify_pcd_on_box(
    pcd: BasicPointCloud, num_new_points: int, scale_box: float = 1.0
):
    """
...
class TriangleModel:
    ...
    def save(self, path):
        ...
        torch.save(
            point_cloud_state_dict, os.path.join(path, "point_cloud_state_dict.pt")
        )
    ...
    def load(self, path):
        ...
        point_cloud_state_dict = torch.load(
            os.path.join(path, "point_cloud_state_dict.pt")
        )
        ...
        self._triangles_points = (
            point_cloud_state_dict["triangles_points"][i : i + plus]
            .to("cuda")
            .to(torch.float32)
            .detach()
            .clone()
        ...
        self._sigma = (
            point_cloud_state_dict["sigma"][i : i + plus]
            .to("cuda")
            .to(torch.float32)
            .detach()
            .clone()
        ...
        self._features_dc = (
            point_cloud_state_dict["features_dc"][i : i + plus]
            .to("cuda")
            .to(torch.float32)
            .detach()
            .clone()
        ...
        self._features_rest = (
            point_cloud_state_dict["features_rest"][i : i + plus]
            .to("cuda")
            .to(torch.float32)
            .detach()
            .clone()
        ...
        self._opacity = (
            point_cloud_state_dict["opacity"][i : i + plus]
            .to("cuda")
            .to(torch.float32)
            .detach()
            .clone()
        ...
        self._mask = nn.Parameter(
            torch.ones(
                (self._triangles_points.size(0), 1), device="cuda"
            ).requires_grad_(True)
        )
        ...
        tensor_num_points_per_triangle = torch.tensor(
            num_points_per_triangle, dtype=torch.int, device="cuda:0"
        )
        ...
        l = [
            ...
            {
                "params": [self._triangles_points],
                "lr": 0.00001,
                "name": "triangles_points",
            },
    ...
    def create_from_pcd(
        ...
        if no_dome:
            ...
            total_colors = np.concatenate(
                [np.ones_like(sky_box_points), pcd_colors], axis=0
            )
        ...
        sigmas = self.inverse_exponential_activation(
            torch.ones((number_of_points, 1), dtype=torch.float, device="cuda")
            * set_sigma
        )
        ...
        self._triangles_points = nn.Parameter(
            points_per_triangle.to("cuda").requires_grad_(True)
        )
        self._sigma = nn.Parameter(sigmas.requires_grad_(True))
        self._num_points_per_triangle = tensor_num_points_per_triangle
        self._cumsum_of_points_per_triangle = cumsum_of_points_per_triangle
        self._number_of_points = number_of_points
        self.max_scaling = torch.zeros(
            (fused_point_cloud.shape[0]), dtype=torch.float, device="cuda"
        )
        self.max_radii2D = torch.zeros(
            (fused_point_cloud.shape[0]), dtype=torch.float, device="cuda"
        )
        self.max_density_factor = torch.zeros(
            (fused_point_cloud.shape[0]), dtype=torch.float, device="cuda"
        )
        self._mask = nn.Parameter(
            torch.ones((fused_point_cloud.shape[0], 1), device="cuda").requires_grad_(
                True
            )
        )
        ...
        self.image_size = torch.zeros(
            (fused_point_cloud.shape[0]), dtype=torch.float, device="cuda"
        )
        self.importance_score = torch.zeros(
            (fused_point_cloud.shape[0]), dtype=torch.float, device="cuda"
        )
    ...
    def reset_sigma(self):
        new_sigma = self.inverse_exponential_activation(
            torch.ones((self._opacity.shape[0], 1), dtype=torch.float, device="cuda")
        )
    ...
    def densification_postfix(
        ...
        self.max_scaling = torch.cat(
            (self.max_scaling, torch.zeros(new_opacities.shape[0], device="cuda")),
            dim=0,
        )
    ...
    def _update_params_small(self, idxs):
        ...
        noise = (
            torch.rand(n, 1, 3, device=new_triangles_points.device) - 0.5
        ...
        return (
            torch.cat(
                [self._triangles_points[idxs], new_triangles_points_noisy], dim=0
            ),
            ...
            torch.cat(
                [self._mask[idxs], torch.ones((n, 1), device=self._mask.device)], dim=0
            ),
        )
    ...
    def _sample_alives(self, probs, num, big_mask, alive_indices=None):
        ...
        sampled_idxs = torch.multinomial(
            probs, min(num, (probs > 0).sum().item()), replacement=False
        )
    ...
    def add_new_gs(self, cap_max, oddGroup=True, dead_mask=None):
        ...
        if num_big > 0:
            ...
            (
                ...
                split_sigma,
                split_mask,
            ) = self._update_params(big_indices)

        else:
            split_triangles_points = torch.empty(
                (0, 3, 3), device=self._triangles_points.device
            )
            split_features_dc = torch.empty(
                (0,) + self._features_dc.shape[1:], device=self._features_dc.device
            )
            split_features_rest = torch.empty(
                (0,) + self._features_rest.shape[1:], device=self._features_rest.device
            )
        ...
        if num_small > 0:
            (
                ...
                clone_sigma,
                clone_mask,
            ) = self._update_params_small(small_indices)

        else:
            clone_triangles_points = torch.empty(
                (0, 3, 3), device=self._triangles_points.device
            )
            clone_features_dc = torch.empty(
                (0,) + self._features_dc.shape[1:], device=self._features_dc.device
            )
            clone_features_rest = torch.empty(
                (0,) + self._features_rest.shape[1:], device=self._features_rest.device
            )
        ...
        new_triangles_points = torch.cat(
            [split_triangles_points, clone_triangles_points], dim=0
        )
    ...
    def get_model_by_indices(self, indices):
        ...
        new_model.max_scaling = torch.zeros(
            (number_of_points), dtype=torch.float, device="cuda"
        )
        new_model.max_radii2D = torch.zeros(
            (number_of_points), dtype=torch.float, device="cuda"
        )
        new_model.max_density_factor = torch.zeros(
            (number_of_points), dtype=torch.float, device="cuda"
        )
