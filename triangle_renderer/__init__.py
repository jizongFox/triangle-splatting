...
def render(
    ...
    screenspace_points = (
        torch.zeros_like(
            pc.get_triangles_points[:, 0, :].squeeze(),
            dtype=pc.get_triangles_points.dtype,
            requires_grad=True,
            device="cuda",
        )
        + 0
    ...
    scaling = torch.zeros_like(
        pc.get_triangles_points[:, 0, 0].squeeze(),
        dtype=pc.get_triangles_points.dtype,
        requires_grad=True,
        device="cuda",
    ).detach()
    ...
    mask = (
        (torch.sigmoid(pc._mask) > 0.01).float() - torch.sigmoid(pc._mask)
    ).detach() + torch.sigmoid(pc._mask)
