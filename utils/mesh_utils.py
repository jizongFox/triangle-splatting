...
def post_process_mesh(mesh, cluster_to_keep=1000):
    ...
    print("post processing the mesh to have {} cluster_to_keep".format(cluster_to_keep))
...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(
            ...
            """depth = depth.cpu().detach().numpy()[0]
            ...
            depth_image = Image.fromarray(colored_depth_rgb)
            depth_image.save("Image/" + str(i) + "depth.png")

            image_array = rgb.detach().permute(1, 2, 0).cpu().numpy()
            image_array = np.clip(image_array, 0, 1)
            plt.imsave("Image/" + str(i) + "image.png", image_array)"""
