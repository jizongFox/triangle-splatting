...
def apply_dog_filter(batch, freq=50, scale_factor=0.5):
    ...
    downscaled = F.interpolate(
        batch, scale_factor=scale_factor, mode="bilinear", align_corners=False
    )
    ...
    upscaled_mask = F.interpolate(
        mask, size=batch.shape[-2:], mode="bilinear", align_corners=False
    )

    upscaled_mask = upscaled_mask - upscaled_mask.min()
    upscaled_mask = (
        upscaled_mask / upscaled_mask.max()
        if freq >= 50
        else 1.0 - upscaled_mask / upscaled_mask.max()
    )
